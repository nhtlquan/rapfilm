...
public class DemoPlayer implements com.google.android.exoplayer.ExoPlayer.Listener, EventListener, HlsSampleSource.EventListener, ExtractorSampleSource.EventListener, SingleSampleSource.EventListener, BandwidthMeter.EventListener, MediaCodecVideoTrackRenderer.EventListener, MediaCodecAudioTrackRenderer.EventListener, StreamingDrmSessionManager.EventListener, DashChunkSource.EventListener, TextRenderer, MetadataRenderer<List<Id3Frame>>, Provider {
    ...
    private final ExoPlayer player = Factory.newInstance(4, 1000, Factory.DEFAULT_MIN_REBUFFER_MS);
    ...
    public void setSelectedTrack(int type, int index) {
        ...
        if (type == 2 && index < 0 && this.captionListener != null) {
            this.captionListener.onCues(Collections.<Cue>emptyList());
        }
    }
    ...
    void onRenderers(TrackRenderer[] renderers, BandwidthMeter bandwidthMeter) {
        ...
        CodecCounters codecCounters = this.videoRenderer instanceof MediaCodecTrackRenderer ? ((MediaCodecTrackRenderer) this.videoRenderer).codecCounters : renderers[1] instanceof MediaCodecTrackRenderer ? ((MediaCodecTrackRenderer) renderers[1]).codecCounters : null;
