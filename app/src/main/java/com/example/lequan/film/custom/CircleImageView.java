...
public class CircleImageView extends ImageView {
    ...
    private void updateShaderMatrix() {
        ...
        if (((float) this.mBitmapWidth) * this.mDrawableRect.height() > this.mDrawableRect.width() * ((float) this.mBitmapHeight)) {
            ...
            dx = (this.mDrawableRect.width() - (((float) this.mBitmapWidth) * scale)) * 0.50f;
        } else {
            scale = this.mDrawableRect.width() / ((float) this.mBitmapWidth);
            dy = (this.mDrawableRect.height() - (((float) this.mBitmapHeight) * scale)) * 0.50f;
        }
        this.mShaderMatrix.setScale(scale, scale);
        this.mShaderMatrix.postTranslate((float) (((int) (dx + 0.50f)) + this.mBorderWidth), (float) (((int) (dy + 0.50f)) + this.mBorderWidth));
